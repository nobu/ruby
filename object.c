...
static ID id_inspect_instance_variables;
 ...
 *
 * If _obj_ responds to +inspect_instance_variables+ method, only
 * instance variables that the name is listed in that array returned
 * by that method will be included in the inspect string.
 *
 *
 ...
 *       end
 *
 *       def inspect_instance_variables = [:@host, :@user]
 *     end
 *
...
{
    VALUE ivars = rb_check_funcall(obj, id_inspect_instance_variables, 0, 0);
    st_index_t n = 0;
    if (UNDEF_P(ivars)) {
        n = rb_ivar_count(obj);
        ivars = Qnil;
...
Init_Object(void)
{
    ...
    id_inspect_instance_variables = rb_intern_const("inspect_instance_variables");
    InitVM(Object);
}
